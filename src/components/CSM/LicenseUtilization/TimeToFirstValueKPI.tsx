...
import { calculateTimeToFirstValueKPI } from '../../../lib/kpis/licenseUtilizationKPIs';

interface TimeToFirstValueKPIProps {
  onDrillDown?: () => void;
  drillDownUrl?: string;
}

export function TimeToFirstValueKPI({ onDrillDown, drillDownUrl }: TimeToFirstValueKPIProps) {
  const [kpi, setKpi] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    try {
      const calculatedKPI = calculateTimeToFirstValueKPI();
      setKpi(calculatedKPI);
      setLoading(false);
    } catch (error) {
      console.error('Error loading Time-to-First-Value KPI:', error);
      setLoading(false);
    }
  }, []);
  ...
  if (!kpi) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="text-red-600">Error loading TTFV data</div>
      </div>
    );
  }
  ...
  const getTrendColor = () => {
    ...
    return kpi.trend === 'down' ? 'text-green-600' : 'text-red-600'; // Down is good for TTFV
  };

  const getPerformanceText = () => {
    if (kpi.value <= 30) return 'Excellent';
    if (kpi.value <= 45) return 'Good';
    if (kpi.value <= 60) return 'Needs Attention';
    return 'Critical';
  };

  const getProgressPercentage = () => {
    // Invert for TTFV - lower is better
    return Math.min((30 / Math.max(kpi.value, 1)) * 100, 100);
  };
