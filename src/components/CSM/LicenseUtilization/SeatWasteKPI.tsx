...
import { calculateSeatWasteKPI } from '../../../lib/kpis/licenseUtilizationKPIs';

interface SeatWasteKPIProps {
  onDrillDown?: () => void;
  drillDownUrl?: string;
}

export function SeatWasteKPI({ onDrillDown, drillDownUrl }: SeatWasteKPIProps) {
  const [kpi, setKpi] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    try {
      const calculatedKPI = calculateSeatWasteKPI();
      setKpi(calculatedKPI);
      setLoading(false);
    } catch (error) {
      console.error('Error loading Seat Waste KPI:', error);
      setLoading(false);
    }
  }, []);
  ...
  if (!kpi) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="text-red-600">Error loading seat waste data</div>
      </div>
    );
  }
  ...
  const getPerformanceText = () => {
    if (kpi.wastePercentage <= 10) return 'Excellent';
    if (kpi.wastePercentage <= 20) return 'Good';
    if (kpi.wastePercentage <= 30) return 'Needs Attention';
    return 'Critical';
  };

  const getProgressPercentage = () => {
    // Invert for waste - lower is better
    return Math.min((kpi.wastePercentage / 50) * 100, 100);
  };
