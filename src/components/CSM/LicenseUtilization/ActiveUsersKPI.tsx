...
import { calculateActiveUsersKPI } from '../../../lib/kpis/licenseUtilizationKPIs';
...
export function ActiveUsersKPI({ onDrillDown, drillDownUrl }: ActiveUsersKPIProps) {
  const [kpi, setKpi] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    try {
      const calculatedKPI = calculateActiveUsersKPI();
      setKpi(calculatedKPI);
      setLoading(false);
    } catch (error) {
      console.error('Error loading Active Users KPI:', error);
      setLoading(false);
    }
  }, []);
  ...
  if (!kpi) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="text-red-600">Error loading active users data</div>
      </div>
    );
  }
  ...
  const getPerformanceText = () => {
    if (kpi.engagementRate >= 70) return 'Excellent';
    if (kpi.engagementRate >= 50) return 'Good';
    if (kpi.engagementRate >= 30) return 'Needs Attention';
    return 'Critical';
  };

  const getProgressPercentage = () => {
    return Math.min((kpi.engagementRate / 100) * 100, 100);
  };
    ...
    >
      <div className="relative h-full flex flex-col">
        ...
        <div className="text-xs text-gray-600 mb-3">
          Engagement: {kpi.engagementRate}% of capacity
        </div>
