...
function ChampionDeparturesContent() {
    ...
    return [...data].sort((a, b) => {
      ...
      switch (sortField) {
        case 'account':
          ...
          break;
        case 'champion':
          aValue = a.departed_contact?.name || '';
          bValue = b.departed_contact?.name || '';
          break;
        case 'departureDate':
          aValue = new Date(a.departure_date || 0).getTime();
          bValue = new Date(b.departure_date || 0).getTime();
          break;
        case 'daysSince':
          aValue = a.daysSinceDeparture || 0;
          bValue = b.daysSinceDeparture || 0;
          break;
        case 'impactScore':
          aValue = a.impact_score || 0;
          bValue = b.impact_score || 0;
          break;
        case 'arrAtRisk':
          aValue = a.arr_at_risk || 0;
          bValue = b.arr_at_risk || 0;
          break;
        case 'status':
          aValue = a.status || '';
          bValue = b.status || '';
    ...
    try {
      ...
      const departureData = championAlerts.map(alert => {
        ...
        return {
          ...
          status: alert.alert_status || 'Open'
        };
      });
      ...
      if (daysParam) {
        ...
        setFilterDays(days);
        // When coming from KPI card, show all statuses, not just "In Progress"
        setFilterStatus('All');
      }
      
      // Initial data will be filtered by useEffect
  ...
  useEffect(() => {
    ...
    // Apply days filter if active
    if (filterDays) {
      filtered = filtered.filter(d => d.daysSinceDeparture <= filterDays);
    }
  ...
  }, [departures, filterDays, filterStatus]);

  const totalPages = Math.ceil(filteredDepartures.length / perPage);
  const paginatedData = sortData(filteredDepartures).slice((currentPage - 1) * perPage, currentPage * perPage);
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-700">
                    {filterDays && `Recent (≤${filterDays} days) • `}
                    {filterStatus !== 'All' && `Status: ${filterStatus} • `}
                    Displaying {filteredDepartures.length} departures
                  </p>
                </div>
              </div>
              <button
