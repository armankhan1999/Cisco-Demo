...
## 15. Dashboard Usage Mapping

### Dashboard Component Inventory

This section maps each new JSON file to specific dashboard components showing **WHERE** and **HOW** the data appears in the UI.

### CSM Portfolio Health Dashboard

| Component | Data Source | Fields Used | Update Frequency |
|-----------|-------------|-------------|------------------|
| **Alert Center Widget** | `intervention_playbooks.json` | status, severity, assigned_csm | Real-time |
| **At-Risk Accounts Table** | `churn_predictions.json` | estimated_days_to_churn, churn_probability | Daily |
| **At-Risk Accounts Table** | `intervention_playbooks.json` | playbook_id (indicator) | Real-time |
| **Champion Departure Banner** | `champion_departure_alerts.json` | alert_id, impact_level, account_name | Real-time |
| **Expansion Ready Accounts** | `expansion_handoff_recommendations.json` | readiness_category="READY NOW" | Weekly |
| **Portfolio Health Score** | *existing* | health_score | Real-time |
| **White-Space Summary** | `white_space_analysis.json` | total_white_space_arr (aggregated) | Quarterly |

---

### Account Drill-Down Dashboard (Level 2)

| Component | Data Source | Fields Used | Update Frequency |
|-----------|-------------|-------------|------------------|
| **Action Panel - Playbook Steps** | `intervention_playbooks.json` | steps[], progress stepper | Real-time |
| **Alert Banner (Top)** | `intervention_playbooks.json` | alert_type, severity, sla_hours | Real-time |
| **Churn Prediction Card** | `churn_predictions.json` | estimated_days, confidence_level, velocity_metrics | Daily |
| **Risk Factor Breakdown** | `churn_predictions.json` | risk_factors[] (donut chart) | Daily |
| **Champion Departure Impact** | `champion_departure_alerts.json` | impact_score, risk_breakdown | On departure |
| **Stakeholder Panel** | `stakeholders.json` (enhanced) | tenure_months, stability_score, linkedin_signals | Daily |
| **Stakeholder Risk Badges** | `stakeholders.json` (enhanced) | tenure_risk_level | Daily |
| **Expansion Opportunity Card** | `expansion_handoff_recommendations.json` | recommended_product, fit_score, estimated_arr | Weekly |
| **White-Space Opportunities** | `white_space_analysis.json` | white_space_opportunities[] (top 3) | Monthly |
| **White-Space ARR Gauge** | `white_space_analysis.json` | total_white_space_arr vs current_arr | Monthly |
| **QBR Value Card** | `qbr_value_stories.json` | executive_summary, total_value_documented | Quarterly |
| **QBR History Timeline** | `qbr_value_stories.json` | qbr_id[], qbr_date, presented_date | Quarterly |
| **Value Metrics Panel** | `qbr_value_stories.json` | security_value, productivity_value, uptime_value | Real-time |
| **Readiness Scorecard** | `multi_product_readiness.json` | component_scores (radar chart) | Weekly |
| **Product Bundle Recommendations** | `product_bundle_recommendations.json` | recommended_bundles[] (top 3) | Monthly |

---

### Health Matrix (Renewal Risk Matrix)

| Component | Data Source | Fields Used | Update Frequency |
|-----------|-------------|-------------|------------------|
| **Cell Detail Tooltip** | `churn_predictions.json` | estimated_days_to_churn | Daily |
| **Color Gradient** | `churn_predictions.json` | intervention_urgency (red gradient) | Daily |
| **Account Bubbles** | *existing* | health_score, renewal_date | Real-time |

---

### CSM Activity Dashboard

| Component | Data Source | Fields Used | Update Frequency |
|-----------|-------------|-------------|------------------|
| **Playbook Adherence KPI** | `intervention_playbooks.json` | completion_percentage (aggregated) | Daily |
| **Playbook Success Rate Chart** | `intervention_playbooks.json` | outcome (saved/churned by type) | Weekly |
| **QBR Prep Time KPI** | `qbr_value_stories.json` | generation_method, prep_time (comparison) | Monthly |
| **Value Documented Chart** | `qbr_value_stories.json` | total_value_documented (summed) | Quarterly |
| **Expansion Handoff Rate** | `expansion_handoff_recommendations.json` | COUNT(handoff_initiated) / month | Monthly |
| **Expansion Handoff Success** | `expansion_handoff_tracking.json` | summary_metrics.engagement_rate | Monthly |
| **AE Performance Table** | `expansion_handoff_tracking.json` | by_ae[] | Monthly |
| **Product Conversion Chart** | `expansion_handoff_tracking.json` | by_product[] win_rates | Quarterly |
| **Handoff Timeline** | `expansion_handoff_tracking.json` | individual_handoffs[] | Real-time |
| **Champion Stability KPI** | `stakeholders.json` (enhanced) | AVG(stability_score) | Weekly |
| **At-Risk Champions Count** | `stakeholders.json` (enhanced) | COUNT(tenure_risk_level) | Weekly |
| **QBR Completion by Tier** | `qbr_tracking.json` | by_tier[].completion_rate | Monthly |
| **QBR Overdue Accounts** | `qbr_tracking.json` | accounts_needing_qbr[] | Weekly |
| **CSM QBR Performance** | `qbr_tracking.json` | by_csm[] | Monthly |
| **Expansion Potential KPI** | `white_space_analysis.json` | AVG(expansion_potential_ratio) | Quarterly |
| **Ready Now Accounts KPI** | `multi_product_readiness.json` | COUNT(readiness_category="READY NOW") | Daily |
| **Readiness Distribution** | `multi_product_readiness.json` | COUNT by readiness_category | Weekly |

---

### Component Type Summary

| Component Type | Count | Primary JSONs Used |
|----------------|-------|-------------------|
| **KPI Cards** | 12 | All JSONs (aggregated metrics) |
| **Tables** | 8 | tracking JSONs, alert JSONs |
| **Charts/Visualizations** | 9 | predictions, handoff tracking, QBR |
| **Alert Banners** | 3 | playbooks, departures |
| **Detail Cards** | 11 | All JSONs (account-level detail) |
| **Progress Indicators** | 4 | playbooks, readiness, handoff |

---

### Update Frequency Matrix

| Frequency | JSON Files | Dashboard Components |
|-----------|-----------|---------------------|
| **Real-time** | intervention_playbooks, champion_departure_alerts | Alert banners, action panels, status badges |
| **Daily** | churn_predictions, stakeholders (enhanced) | Prediction cards, risk indicators, tenure badges |
| **Weekly** | expansion_handoff_recommendations, multi_product_readiness, qbr_tracking | Opportunity cards, readiness scores, overdue lists |
| **Monthly** | white_space_analysis, product_bundle_recommendations | White-space cards, bundle recommendations |
| **Quarterly** | qbr_value_stories, qbr_tracking, expansion_handoff_tracking | QBR summaries, conversion funnels, value trends |
...
This roadmap organizes the 11 gaps into phases based on priority, dependencies, and estimated effort.

---

### Phase 1: Critical Foundation (Gaps 1-2)

**Duration:** 4 weeks  
**Effort:** 80 hours  
**Priority:** 🔴 CRITICAL

| Gap | JSON File | Effort | Key Deliverables |
|-----|-----------|--------|------------------|
| **Gap 1** | `intervention_playbooks.json` | 32h | Playbook library (89 types), Rule engine, Step tracking API, Alert center UI |
| **Gap 2** | `qbr_value_stories.json` | 48h | Value calculation engine, LLM integration, PDF generator, QBR card UI |

**Dependencies:**
- None - can start immediately
- Both integrate with existing `accounts.json`

**Success Criteria:**
- Playbook adherence rate >80% within first month
- QBR prep time reduced by 60% (target: 80%)
- 50+ playbooks triggered in first 2 weeks
- 10+ auto-generated QBRs delivered

**Technical Requirements:**
- Backend: Python/FastAPI for APIs
- ML: Claude API for QBR narrative generation
- Frontend: React components (stepper, value cards)
- Integration: Gainsight webhook, Product APIs

---

### Phase 2: High-Priority Intelligence (Gaps 3-7)
...
| Gap | JSON File | Effort | Key Deliverables |
|-----|-----------|--------|------------------|
| **Gap 3** | `churn_predictions.json` | 60h | ML model training, Prediction API, Time-to-churn UI, Risk factor charts |
| **Gap 4** | `champion_departure_alerts.json` | 40h | LinkedIn integration, Impact scoring, Departure detection, Alert banners |
| **Gap 5** | `expansion_handoff_recommendations.json` | 36h | Readiness algorithm, AE assignment, Talking points generator, Handoff cards |
| **Gap 6** | `expansion_handoff_tracking.json` | 32h | Handoff aggregation, Salesforce webhook, AE leaderboard, Conversion funnel |
| **Gap 7** | `white_space_analysis.json` | 44h | Fit scoring (6 factors), Peer benchmark engine, ARR estimation, White-space cards |

**Dependencies:**
- Gap 4 depends on Gap 11 (stakeholder enhancements)
- Gap 6 depends on Gap 5 (handoff recommendations)
- Gaps 3, 5, 7 can run in parallel

**Implementation Sequence:**
1. **Weeks 5-6:** Gaps 3 + 11 (parallel - both foundational)
2. **Weeks 7:** Gap 5 (enables Gap 6)
3. **Weeks 7-8:** Gaps 4, 6, 7 (parallel)

**Success Criteria:**
- Churn prediction accuracy >80%
- Champion departure detection <14 days
- Handoff engagement rate >75%
- White-space identification >60% of accounts

**Technical Requirements:**
- ML: Gradient boosting model (scikit-learn/XGBoost)
- Integration: LinkedIn Sales Navigator API, Salesforce API
- Database: Peer benchmark database, Historical adoption DB
- Frontend: D3.js charts, radar charts, funnel visualizations

---

### Phase 3: Optimization & Completeness (Gaps 8-11)
...
| Gap | JSON File | Effort | Key Deliverables |
|-----|-----------|--------|------------------|
| **Gap 8** | `multi_product_readiness.json` | 24h | Composite scoring, Component radar chart, Readiness dashboard |
| **Gap 9** | `product_bundle_recommendations.json` | 20h | Pairing analysis, Bundle cards, Historical adoption engine |
| **Gap 10** | `qbr_tracking.json` | 20h | Compliance calculation, Overdue alerts, QBR performance dashboard |
| **Gap 11** | `stakeholders.json` (enhance) | 20h | LinkedIn sync, Tenure calculation, Stability scoring, Risk badges |

**Dependencies:**
- Gap 8 depends on Gaps 5, 7 (uses readiness components)
- Gap 9 can run independently
- Gap 10 depends on Gap 2 (QBR tracking requires QBR generation)
- Gap 11 is foundational for Gap 4 (must complete first)

**Implementation Sequence:**
1. **Week 9:** Gap 11 (foundational for Phase 2)
2. **Week 10:** Gap 8 + 9 (parallel)
3. **Week 11:** Gap 10
4. **Week 12:** Testing, refinement, documentation

**Success Criteria:**
- Average readiness score >75
- Bundle recommendation accuracy >70%
- QBR completion rate >85% across all tiers
- LinkedIn coverage >80%

**Technical Requirements:**
- Integration: LinkedIn API daily sync
- Backend: Aggregation engines, Compliance tracking
- Frontend: Scorecard components, Compliance tables
- Database: Historical pairing database
...
#### Data Infrastructure
- **Snowflake:**
  - Create 10 new tables (one per JSON)
  - Enhance `stakeholders` table (5 new columns)
  - Write transformation jobs (dbt or Python)
  - Synthetic data generation scripts

- **API Layer:**
  - RESTful endpoints for each JSON (CRUD)
  - Real-time webhook listeners (Gainsight, LinkedIn, Salesforce)
  - Batch processing jobs (nightly calculations)

#### Frontend Components
- **Component Library:**
  - 12 new reusable React components
  - D3.js chart library (5 chart types)
  - Material-UI integration (steppers, cards, badges)

#### Integration Points
| External System | Purpose | Gaps Using |
|----------------|---------|-----------|
| Gainsight | Alert triggering | Gap 1 |
| LinkedIn Sales Navigator | Champion tracking, departure detection | Gaps 4, 11 |
| Salesforce | Opportunity tracking | Gap 6 |
| Product APIs (Duo, Umbrella, Meraki, ThousandEyes) | Usage data, security events | Gap 2 |
| Peer Benchmark DB | Industry comparisons | Gaps 5, 7, 9 |
...
|------|---------|---------|---------|
| **Backend Engineer** | 1 FTE | 1 FTE | 0.5 FTE |
| **ML Engineer** | - | 1 FTE | - |
| **Frontend Engineer** | 1 FTE | 1 FTE | 0.5 FTE |
| **Integration Specialist** | 0.5 FTE | 1 FTE | 0.5 FTE |
| **Data Engineer** | 0.5 FTE | 0.5 FTE | 0.5 FTE |

**Total Effort:** 376 hours (~9.5 weeks for 1 developer, or 4 weeks with 2.5 FTE)
...
|------|------------|--------|------------|
| **ML model accuracy <85%** | Medium | High | Start Gap 3 early, allocate 2 weeks for tuning |
| **LinkedIn API rate limits** | High | Medium | Implement caching, batch requests, daily sync |
| **Salesforce integration delays** | Medium | Medium | Mock data for development, parallel testing |
| **Synthetic data quality** | Low | High | Validate against real data samples, CSM review |
| **UI performance with large datasets** | Medium | Medium | Implement pagination, lazy loading, data caching |
...
#### Phase-Based Testing
1. **Phase 1:** Unit tests (80% coverage), Integration tests (Gainsight webhook), UAT with 5 CSMs
2. **Phase 2:** ML model validation (cross-validation), API load testing, UAT with 10 CSMs
3. **Phase 3:** End-to-end testing, Performance testing (1000+ accounts), Full CSM team UAT

#### Acceptance Criteria
- **Functionality:** All KPIs calculable, all dashboards rendering
- **Performance:** Page load <2s, API response <500ms
- **Accuracy:** Predictions within 10% of actual, fit scores validated by CSMs
- **Usability:** CSM task completion time reduced by 50%
...
#### Rollout Plan
1. **Week 1-4 (Phase 1):** Deploy to 5 pilot CSMs, iterate based on feedback
2. **Week 5-12 (Phase 2):** Deploy to 25 CSMs (50% of team), monitor performance
3. **Week 13-16 (Phase 3):** Full team rollout (50 CSMs), enterprise-wide

#### Monitoring
- **Data Quality:** Daily checks on JSON completeness, field validation
- **Usage Metrics:** Dashboard views, KPI usage, feature adoption
- **Performance:** API latency, page load times, error rates
- **Business Impact:** Churn rate, expansion ARR, CSM productivity
...
|--------|----------|--------|------------------|
| **Churn Rate** | 12% | 8% | 6 months post-launch |
| **Expansion ARR** | $5M/year | $8M/year | 6 months post-launch |
| **CSM Productivity** | 40 accounts/CSM | 60 accounts/CSM | 3 months post-launch |
| **QBR Prep Time** | 16h | 3h | 1 month post-launch |
| **Playbook Adherence** | 45% | 90% | 2 months post-launch |
| **White-Space Identification** | 0% | 65% | 3 months post-launch |
